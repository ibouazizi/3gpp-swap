...
function generateSourceId(prefix = 'src') {
  // Ensure at least 10 UTF-8 characters
  const id = `${prefix}-${v4().replace(/-/g, '')}`;
...
class RegisterMessage extends SwapMessage {
...
class ResponseMessage extends SwapMessage {
...
class ConnectMessage extends SwapMessage {
...
class AcceptMessage extends SwapMessage {
...
class RejectMessage extends SwapMessage {
...
class UpdateMessage extends SwapMessage {
...
class CloseMessage extends SwapMessage {
...
class ApplicationMessage extends SwapMessage {
...
class WebSocketTransport extends Emitter {
  constructor(options = {}) {
    super();
  ...
  async _getWebSocketCtor() {
    ...
    try {
      const mod = await Promise.resolve().then(function () { return wsEmpty; });
...
class SwapClient extends Emitter {
  constructor(options = {}) {
    super();
    this.options = options;
...
var wsEmpty = /*#__PURE__*/Object.freeze({
  __proto__: null,
  default: undefined$1
});

export { AcceptMessage, ApplicationMessage, CloseMessage, ConnectMessage, CriteriaBuilder, Encryption, ErrorTypes, IceGatherer, IntegrityProtection, KeyDerivation, Logger, MatchingEngine, MessageFactory, MessageQueue, MessageTypes, ProblemDetails, ReconnectionManager, RegisterMessage, RejectMessage, ResponseMessage, SdpUtils, SdpValidator, SecurityManager, SessionManager, StateMachine, States, SwapClient, SwapError, SwapMessage, SwapWebRTCHelper, UpdateMessage, WebSocketTransport, buildSwapUri, defaultLogger, generateSourceId, getSchemaFor, nextMessageId, validateMessageShape };
